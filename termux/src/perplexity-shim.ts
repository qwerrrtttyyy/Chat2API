/**
 * Perplexity Adapter for Termux
 * Replaces Electron's net.request with axios for HTTP requests.
 * All other logic (message extraction, model mapping, session management) is preserved.
 */
...
function mapModel(model: string): string {
  const directMappings: Record<string, string> = {
    'Auto': 'turbo', 'Turbo': 'turbo', 'PPLX-Pro': 'pplx_pro',
    'GPT-5': 'gpt5', 'Gemini-2.5-Pro': 'gemini25pro',
    'Claude-Sonnet-4': 'claude4sonnet', 'Claude-Opus-4': 'claude4opus',
    'Nemotron': 'nemotron',
  }
  ...
  if (legacyMappings[modelLower]) return legacyMappings[modelLower]
...
function formatNetworkError(error: Error): string {
  ...
  if (errorMsg.includes('ERR_CONNECTION_RESET') || errorMsg.includes('ECONNRESET'))
  ...
  if (errorMsg.includes('ERR_CONNECTION_TIMED_OUT') || errorMsg.includes('ETIMEDOUT'))
  ...
  if (errorMsg.includes('ERR_NAME_NOT_RESOLVED') || errorMsg.includes('ENOTFOUND'))
...
export class PerplexityAdapter {
  ...
  private buildRequestData(query: string, model: string): any {
    ...
    const baseParams: any = {
      ...
      search_focus: 'internet', sources: ['web'], search_recency_filter: null,
      frontend_uuid: frontendUuid, mode: 'copilot', model_preference: model,
      is_related_query: false, is_sponsored: false,
      frontend_context_uuid: frontendContextUuid,
      prompt_source: 'user', query_source: 'home', is_incognito: false,
      time_from_first_type: 18361, local_search_enabled: false,
      ...
      override_no_search: false, should_ask_for_mcp_tool_confirmation: true,
    ...
    return { params: baseParams, query_str: query }
  }
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ stream: Readable; sessionId: string }> {
    ...
    if (!query) throw new Error('No user message found in request')
    ...
    try {
      ...
      if (statusCode >= 400) {
        let errorBody = ''
        const respStream = response.data as Readable
        respStream.on('data', (chunk: Buffer) => { errorBody += chunk.toString() })
        await new Promise<void>((resolve) => respStream.on('end', resolve))
        const errorMsg = `HTTP ${statusCode}: ${errorBody.substring(0, 200)}`
        stream.emit('error', new Error(errorMsg))
        throw new Error(errorMsg)
      }

      // Success - pipe response data to stream
      const respStream = response.data as Readable
      respStream.on('data', (chunk: Buffer) => { stream.push(chunk) })
      respStream.on('end', () => { stream.push(null) })
      respStream.on('error', (error: Error) => {
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    ...
    try {
      ...
      const headers: Record<string, string> = {
        ...
        'Cookie': this.buildCookieHeader(), 'Origin': PERPLEXITY_URL,
        'Referer': `${PERPLEXITY_URL}/`,
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36',
        'sec-ch-ua': '"Chromium";v="134"', 'sec-ch-ua-mobile': '?0',
        'sec-ch-ua-platform': '"Linux"', 'sec-fetch-dest': 'empty',
        'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin',
        'x-app-apiclient': 'default', 'x-app-apiversion': '2.18',
      ...
      const response = await axios({
        ...
        url: deleteUrl,
        headers,
        data: { entry_uuid: sessionData.backend_uuid, read_write_token: sessionData.read_write_token || '' },
        validateStatus: () => true,
      })
      ...
      console.error('[Perplexity-Termux] Delete session error:', error)
      sessionCache.delete(cacheKey)
      return false
    }
  }
  ...
  async deleteAllChats(): Promise<boolean> {
    ...
    try {
      ...
      console.error('[Perplexity-Termux] Delete all chats error:', error)
      return false
    }
  }
